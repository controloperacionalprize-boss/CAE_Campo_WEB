...
CREATE TABLE IF NOT EXISTS viaje (
    ...
    conductor_id INTEGER REFERENCES chofer (id),
    ...
    estado VARCHAR(20) NOT NULL DEFAULT 'en_proceso' CHECK (estado IN ('en_proceso', 'finalizado', 'anulado')),
...
CREATE TABLE IF NOT EXISTS viaje_detalle (
    id SERIAL PRIMARY KEY,
    viaje_id INTEGER NOT NULL REFERENCES viaje (id) ON DELETE CASCADE,
    guia_ingreso_id INTEGER NOT NULL REFERENCES guia_ingreso (id),
...
CREATE TABLE IF NOT EXISTS croquis (
    id SERIAL PRIMARY KEY,
    viaje_id INTEGER NOT NULL UNIQUE REFERENCES viaje (id) ON DELETE CASCADE,
    fecha DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS croquis_pallet (
    id SERIAL PRIMARY KEY,
    croquis_id INTEGER NOT NULL REFERENCES croquis (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS grr (
    id SERIAL PRIMARY KEY,
    viaje_id INTEGER NOT NULL UNIQUE REFERENCES viaje (id),
...
CREATE TABLE IF NOT EXISTS grr_detalle (
    id SERIAL PRIMARY KEY,
    grr_id INTEGER NOT NULL REFERENCES grr (id) ON DELETE CASCADE,
